...
function Components:NewSelector(text, default, list, callback, multiselect)
    text = text or "selector"
    default = default or (multiselect and {} or ". . .")
    list = list or {}
    callback = callback or function() end
    ...
    selectorContainer.ChildRemoved:Connect(UpdatePageSize)

    UpdatePageSize()

    -- Track selected items - use table format like Fluent {option = true}
    ...
    local function updateDisplay()
        if multiselect then
            ...
            if #displayText == 0 then
                ...
                selectorText.Text = table.concat(displayText, ", ")
            end
        else
            selectorText.Text = selectedValues or ". . ."
        end
    end
    ...
    for i,v in next, list do
        ...
        if multiselect then
            ...
            checkBoxCorner = Instance.new("UICorner")
            checkMark = Instance.new("Frame")
            checkMarkCorner = Instance.new("UICorner")
            ...
            checkMark.BackgroundColor3 = Color3.fromRGB(159, 115, 255)
        ...
        optionButton.MouseButton1Click:Connect(function()
            if multiselect then
                -- Toggle selection
                selectedValues[v] = not selectedValues[v] or nil
                
                -- Update checkbox visual
                if selectedValues[v] then
                    TweenService:Create(checkMark, TweenTable["selector"], {Size = UDim2.new(0, 10, 0, 10)}):Play()
                    TweenService:Create(optionButton, TweenTable["selector"], {TextColor3 = Color3.fromRGB(159, 115, 255)}):Play()
                else
                    TweenService:Create(checkMark, TweenTable["selector"], {Size = UDim2.new(0, 0, 0, 0)}):Play()
                ...
                -- Single select - update all buttons
    ...
    function SelectorFunctions:AddOption(new)
        new = new or "option"
        ...
        optionButton.MouseButton1Click:Connect(function()
            if multiselect then
                selectedValues[new] = not selectedValues[new] or nil
                
                if selectedValues[new] then
                    TweenService:Create(checkMark, TweenTable["selector"], {Size = UDim2.new(0, 10, 0, 10)}):Play()
                    TweenService:Create(optionButton, TweenTable["selector"], {TextColor3 = Color3.fromRGB(159, 115, 255)}):Play()
                else
                    TweenService:Create(checkMark, TweenTable["selector"], {Size = UDim2.new(0, 0, 0, 0)}):Play()
                    TweenService:Create(optionButton, TweenTable["selector"], {TextColor3 = Color3.fromRGB(140, 140, 140)}):Play()
                end
    ...
    function SelectorFunctions:RemoveOption(option)
        ...
        elseif selectedValues == option then
            selectedValues = nil
        end
        ...
        for i,v in pairs(selectorContainer:GetDescendants()) do
            if v:IsA("TextButton") then
                local optionText = multiselect and v.Text:gsub("^%s+", "") or v.Text
    ...
    function SelectorFunctions:GetSelected()
        return selectedValues
    end
    
    function SelectorFunctions:SetValue(val)
        if multiselect then
            selectedValues = {}
        ...
        -- Update all buttons
        for i,v in pairs(selectorContainer:GetChildren()) do
            if v:IsA("TextButton") then
                local optionText = multiselect and v.Text:gsub("^%s+", "") or v.Text
                if multiselect then
                    local checkMark = v:FindFirstChild("checkBox") and v.checkBox:FindFirstChild("checkMark")
                    if checkMark then
                        if selectedValues[optionText] then
                            checkMark.Size = UDim2.new(0, 10, 0, 10)
                            v.TextColor3 = Color3.fromRGB(159, 115, 255)
                        else
                            checkMark.Size = UDim2.new(0, 0, 0, 0)
                    ...
                    if optionText == selectedValues then
                        v.TextColor3 = Color3.fromRGB(159, 115, 255)
                    else
                        v.TextColor3 = Color3.fromRGB(160, 160, 160)
                    end
